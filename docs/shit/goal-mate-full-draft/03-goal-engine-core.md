...
从老三论视角看，Goal Engine Core 是系统的控制器：

```text
系统论：维护目标、条件、行动、反馈、资源和约束之间的系统状态。
信息论：把用户自然语言、日志和 Check-in 压缩成可计算的结构化状态。
控制论：根据目标偏差选择下一次干预，包括澄清、拆小、换路、提醒、取舍和复盘。
```

因此 Goal Engine 不能只生成计划，还必须持续回答：
