...
v0.1 必须支持模型配置，默认加入 B.AI 兼容 OpenAI 接口。

| 字段 | 必填 | 示例 | 说明 |
| --- | --- | --- | --- |
| provider | 是 | B.AI | 模型供应商 |
| model | 是 | gpt-5-nano | 默认便宜快速模型 |
| reasoning_model | 否 | 可选 reasoning model | 复杂目标推理可用 |
| api_base | 是 | https://api.b.ai | API 地址 |
...
服务器 `.env` 可以保留 B.AI 默认 API Base / 默认模型等非密钥参数，但不能把服务器全局模型 API Key 当作所有用户共用的默认密钥。Web Agent、QQ Agent、Scheduler 和模型测试都必须优先使用当前 session 用户保存的模型密钥。
...
| 编号 | Given | When | Then |
| --- | --- | --- | --- |
| AC-F5-1 | 用户打开 Settings | 查看 Models | 能看到 B.AI、模型、API Base、API Key 和测试连接 |
