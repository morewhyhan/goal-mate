...
| Live model Agent flow | 检查用户保存 B.AI Key 后，Settings 测试和 Agent 发消息是否真实调用模型 | `pnpm verify:live-model-agent:write` |
...
| Proactive contact policy | 用纯函数和 fake Prisma 检查 consent、候选窗口、send / skip / defer、暂停与恢复边界 | `pnpm test` |
...
- QQ shared runtime。
- Web / QQ 同一逻辑 runtime 和跨渠道最近记忆。
- Scheduler shared audit。
- QQ 绑定与主动联系 consent 分离。
- 主动联系默认关闭、启用 / 恢复须确认、暂停立即生效且不得自动恢复。
- Contact Policy send / skip / defer，以及目标状态、目标范围完成、行动窗口、最近反馈、免打扰、待回复、静默、限流抑制。
- check-in diagnosis。
- 明确 Check-in 无二次确认，并且只绑定当前主目标的今日行动。
- 反馈事务持久化 nextCommitment；QQ 只有实际落库后才声称已经调整。
- PROMPT 调时只修改已有 enabled + consented 规则，不创建、不启用、不增频。
- Today auto action。
- Momentum heatmap。
- Markdown rollups。
...
真实模型验证会调用默认 B.AI 配置，检查少量 live 回复是否通过同一质量门禁。它能证明当前模型在样本上的基本回复质量，但不能替代大规模人工评测、长期对话质量评测或真实用户体验评测。
...
- 2026-07-03 已使用 `RUN_REAL_LIVE_AI=1 pnpm verify:ai-reply-quality` 跑通当时配置的真实模型回复质量验证。
...
- 2026-07-04 `pnpm verify:ai-reply-quality` 已补齐秘书式对话 5 类样本门禁：模糊目标、今日行动、没做反馈、复盘、设置读取。该门禁证明本地质量基线覆盖核心体验，不替代真实模型长期质量验收。

## 3.5.1 Live Model Agent Flow 验证

`pnpm verify:live-model-agent` 用于证明真实用户模型配置链路成立，而不是只证明脚本能直接调用 B.AI。

该验证必须覆盖：

- 注册一个干净测试用户。
- 通过 `/api/models` 为当前用户保存 B.AI API Key，并确认响应不泄露明文。
- 调用 `/api/settings/models/test`，证明 Settings 的测试按钮等价链路能访问 B.AI。
- 如果 B.AI 返回余额不足、Key 无效、限流或网络失败，系统必须返回结构化 `reason` 和用户能理解的 `message`，不能把原始 JSON 错误直接丢给用户。
...
也可以使用 `BAI_API_KEY`、`OPENAI_API_KEY` 或历史兼容的 `DEEPSEEK_API_KEY` 环境变量。报告不得写入 API Key。

该验证是强 live 证据；如果没有真实 key 或网络权限，不能声明“用户配置 B.AI 后 Agent 真实调用 AI”已经通过。
...
该验证不访问外部模型，不证明真实模型长期回复质量。它证明的是 Agent Runtime 的上下文注入、权限边界和用户隔离在真实 API 路径上成立。
...
该验证使用本地 fake model 和 fake QQ token server，不证明外部 B.AI 余额或真实 QQ Gateway 长期送达。
...
- 验证关闭的提醒规则不会被消费。
- 验证当天达到全局或单规则上限时决定 `skip:daily_limit`，不发送消息。
- 验证命中 `quietHours` 时决定 `defer:quiet_hours`，不发送消息。
...
这组能力可以在不访问真实 QQ 和真实模型的情况下完成逻辑验收：
...
`pnpm test` 中与主动联系相关的纯策略 / fake Prisma 场景必须证明：

- 干净用户的推荐候选规则默认关闭。
- QQ 绑定不是 consent；全局 consent 和规则 consent 缺一不可。
- “你看着合适的时候提醒我”会形成 autonomous reminder 待确认动作。
- 未确认前不启用规则；确认后才写入全局授权和候选窗口 metadata。
- “暂停 / 别提醒”立即撤销全局授权并关闭规则，不要求二次确认。
- 暂停会使更早的待确认启用失效，普通聊天不会自动恢复。
- 恢复主动联系需要新的确认。
- Contact Policy 对规则关闭、拒绝授权、缺少 consent、目标非活动、目标范围内已完成、缺少相关行动、尚未到行动窗口、最近刚反馈、免打扰、待回复、连续无响应、每日限流、QQ context 授权歧义和平台上下文分别输出可解释的 send / skip / defer。
- 中午提示只有在存在启动风险或恢复窗口时才发送；周复盘只有存在新活动或未闭合条件时才发送。
- cadence 三档使用不同候选窗口，周日有价值的周复盘不会被普通检查提前耗尽触达容量。
- consent 必须绑定明确 QQ context；禁止回退到最近绑定，缺少 / 歧义 / 错配均不能发送。
- QQ 普通聊天不会仅因为 18 小时内存在 sent reminder 就被误判为 Scheduler 反馈。
- 明确完成 / 部分完成 / 没做反馈直接写 Check-in，不产生第二次确认。
- 未带 `actionId` 的反馈只匹配当前 ACTIVE 主目标的今日可执行行动。
- `DONE` 产生次日下一承诺并可转向下一个缺口；`PARTIAL / NOT_DONE` 产生同日 2–10 分钟的更小 `PLANNED` 行动。
- PROMPT / MOTIVATION / PATH 分别形成提示预案、目标真实性确认、关键条件重对准行动。
- PROMPT 只修改已有 enabled + consented QQ 规则的 schedule，保留 previous/new/sourceDiagnosis，不修改 enabled 或 maxPerDay。

这组验证回答“系统逻辑上会不会未经同意打扰、该停时会不会停、Web / QQ 是否在操作同一套状态”。它不回答真实 QQ OpenAPI 当前是否授予主动消息权限、服务器是否长期在线、某条消息是否真的送达。后一组属于外部运行验收，本轮不要求执行，也不能在没有证据时宣称通过。

Scheduler 本地 verifier 还应证明 quiet-hours defer 会写入未来
`scheduledFor`，到期后 claim 同一 `SchedulerEvent`、保留 defer history 并重跑完整 Contact Policy，而不是丢失 dueKey 或直接绕过条件发送。

截至 2026-07-24，本轮纯逻辑测试共 `53/53` 通过，静态验收全部通过；Prompt
snapshot 为 `goal-mate-agent-system-v0.9.1`，source hash 为
`4986380659ed17ea1300532213e5950a4976a5d1de0bb94ea64fd1411d8b727c`。
这些结果证明本地逻辑和静态契约，不等于 live QQ 送达或服务器长期运行已经验收。
