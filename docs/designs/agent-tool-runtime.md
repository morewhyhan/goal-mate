...
- Internal scheduler action：`reminder.send` 是 Scheduler 主动发送提醒时写入的内部审计动作，不出现在用户可调用工具目录中；它通过 shared audit writer 写入 `AgentToolAction`。
...
- `pnpm verify:agent-control` 覆盖 `settings.model.update` 和 `reminder.schedule`。
- 两个工具默认都必须先生成 `pending_confirmation`。
- 用户确认后才写入当前用户的 `ModelConfig` 或 `ReminderRule`。
- 模型密钥必须加密保存；读取、导出和 Settings Control Center 不能泄露明文。
- `settings.model.update` 的待确认动作和工具审计也不能保存明文 `apiKey`；进入 `AgentToolAction.input` 前必须转成加密 `apiKeyRef`。
- Settings Control Center 必须展示 Agent 写入后的模型、提醒规则和 `AgentToolAction` 审计。
